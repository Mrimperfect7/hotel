...
const NAV = [
  ...
  { href: '/guide/bookings', label: 'Bookings', icon: '📅' },
  { href: '/guide/profile', label: 'My Profile', icon: '⚙️' },
];
...
export default function GuideLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!getToken()) { router.replace('/login?next=/guide'); return; }
    api.get<{ user: { role: string } }>('/api/auth/me', true)
      .then((r) => {
        if (!['GUIDE', 'ADMIN', 'SUPER_ADMIN'].includes(r.user.role)) {
          router.replace('/login?next=/guide');
        } else {
          setChecked(true);
        }
      })
      .catch(() => router.replace('/login?next=/guide'));
  }, [router]);
