...
const NAV = [
  ...
  { href: '/driver/rides', label: 'My Rides', icon: '🛣️' },
  { href: '/driver/profile', label: 'Profile & Vehicle', icon: '⚙️' },
];
...
export default function DriverLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!getToken()) { router.replace('/login?next=/driver'); return; }
    api.get<{ user: { role: string } }>('/api/auth/me', true)
      .then((r) => {
        if (!['DRIVER', 'ADMIN', 'SUPER_ADMIN'].includes(r.user.role)) {
          router.replace('/login?next=/driver');
        } else {
          setChecked(true);
        }
      })
      .catch(() => router.replace('/login?next=/driver'));
  }, [router]);
